/* Chip-Osc — the Hook page's engine (app.js), trimmed to what a synth page needs. Runs two ways.
...
const host = { top: 0, vh: 0, top0: null, ready: false, nav: false, phone: false, adminH: 0, vel: 0, t: 0, settle: 0 };   /* adminH: live coverage of the WP admin bar (0 for visitors); nav: the host draws the menu bar itself */
/* Pinned things are placed from host messages that arrive a frame or two after the host has scrolled, which reads
   as jitter. lead() predicts where the host will be by paint time from the scroll velocity; when scrolling stops
   the velocity is zeroed so everything settles exactly. */
const LEAD_MS = 24;
...
function postWalk() {
  ...
  parent.postMessage({ hook: "walk", top: w.top + y0, bottom: w.bottom + y0, left: p.left, width: p.width, aspect: 1100 / 904, navH: NAV_H, marginFrac: 0.16, steps, hl }, "*");
}
...
(() => {
  ...
  scrollHandlers.push(() => {
    if (host.ready) {
      ...
      nav.style.transform = `translate3d(0,${Math.max(0, -(host.top + lead())) + host.adminH}px,0)`;
